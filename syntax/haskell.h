#ifndef __SYNTAX_HS
#define __SYNTAX_HS

#include <string.h>
#include <ctype.h>
#include "parser.h"

static const char *_HS_EXTENSIONS[] = {".hs", NULL};
static const char *_HS_KEYWORDS[] = {
    "case", "of", "if", "then", "else", "data", "in",
    "otherwise", "class", "where", "do", "let", "as",
    "module", "import", "hiding", "instance", "return",
    "type", "newtype", "forall", "deriving", NULL,
};

#define LEX_INC(AMOUNT) { tok.end += AMOUNT; lex->cur += AMOUNT; }

static bool _hs_is_operator(syntax_t *syntax, char c) {
    return strchr("+-*/%!@=<>.,&|(){}[]~^\\?:;$", c) != NULL;
}

static inline bool _hs_is_num(char c) {
    return strchr("xXbBoO.", c) != NULL || isxdigit(c);
}

static token_t _hs_next_token(syntax_t *syntax, lexer_t *lex) {
    token_t tok = {0};
    syntax->skip_space(syntax, lex);
    if (lex->cur >= lex->text_sz) return tok;

    tok.start = tok.end = lex->cur;
    if (lex->cur+2 < lex->text_sz && strncmp(lex->text+lex->cur, "--", 2) == 0) {
        tok.type = LTK_COMMENT;
        while (lex->text[lex->cur] != '\n' && lex->cur < lex->text_sz) LEX_INC(1);
        return tok;
    }
    if (lex->cur+2 < lex->text_sz && strncmp(lex->text+lex->cur, "{-", 2) == 0) {
        tok.type = LTK_COMMENT;
        LEX_INC(2);
        while (lex->cur+2 < lex->text_sz && strncmp(lex->text+lex->cur, "-}", 2) != 0)
            LEX_INC(1);
        LEX_INC(2);
        return tok;
    }
    if (lex->text[lex->cur] == '\"' /*|| lex->text[lex->cur] == '\''*/ || lex->text[lex->cur] == '`') {
        char match = lex->text[lex->cur];
        tok.type = LTK_LITERAL;
        do {
            int amnt = (lex->text[tok.end] == '\\')? 2 : 1;
            LEX_INC(1);
        } while (lex->text[lex->cur] != match && lex->cur < lex->text_sz);
        LEX_INC(1);
        return tok;
    }
    if (isdigit(lex->text[lex->cur])) {
        tok.type = LTK_LITERAL;
        while (_hs_is_num(lex->text[lex->cur]) && lex->cur < lex->text_sz) LEX_INC(1);
        return tok;
    }
    if (syntax->is_operator(syntax, lex->text[lex->cur])) {
        tok.type = LTK_OPERATOR;
        LEX_INC(1);
        return tok;
    }
    tok.type = LTK_IDENTIFIER;
    do LEX_INC(1)
    while (syntax->is_word(syntax, lex->text[lex->cur]) && lex->cur < lex->text_sz);
    if (default_is_keyword(syntax, lex, tok)) tok.type = LTK_KEYWORD;
    if (isupper(lex->text[tok.start])) tok.type = LTK_KEYWORD;
    return tok;
}

#undef LEX_INC

syntax_t syntax_haskell(void) {
    return (syntax_t) {
        .name        = "haskell",
        .extensions  = _HS_EXTENSIONS,
        .keywords    = _HS_KEYWORDS,
        .is_operator = _hs_is_operator,
        .is_word     = default_is_word,
        .next_token  = _hs_next_token,
        .skip_space  = default_skip_space,
    };
}

#endif
